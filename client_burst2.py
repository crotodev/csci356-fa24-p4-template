...
# delay between the beginning of each burst, i.e. one burst per T seconds.
...
#           go to state 2.
#   state 1. Wait long enough so next burst starts T seconds after the last one
#           started.
...
#   python3 client_burst2.py 1.2.3.4 6000 50 0.120
# This will send data to a server at IP address 1.2.3.4 port 6000, with N=50
# packets per burst, and T=0.120 seconds (or 120 ms) per burst.
...
# sleep until the specified target_time. If the target_time has already passed,
# this function does nothing. Examples:
#   sleep_until(start + 3.5)   -- this waits until 3.5 seconds after the start time
#   sleep_until(start + k*0.5) -- this waits until k*0.5 seconds after the start time
#   sleep_until(start + b*t)   -- this waits until b*t seconds after the start time
...
if __name__ == "__main__":
    if len(sys.argv) <= 4:
        ...
        print("   python client_burst2.py 1.2.3.4 6000 50 0.120")
        sys.exit(0)
    host = sys.argv[1]
    port = int(sys.argv[2])
    n = int(sys.argv[3])
